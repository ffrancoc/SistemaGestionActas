...
class Registro(models.Model):
    ...
    def to_dict(self):
        return {
            ...
            'creacion': timezone.localtime(self.creacion).strftime('%Y-%m-%d %H:%M:%S') if self.creacion else '',
            'modificacion': timezone.localtime(self.modificacion).strftime('%Y-%m-%d %H:%M:%S') if self.modificacion else '',
            'estado': self.estado
        }
...
class Bautismo(models.Model):
    registro = models.OneToOneField('Registro', on_delete=models.CASCADE, related_name='bautismos') 
    id = models.AutoField(primary_key=True)
    ...
    def to_dict(self):
        return {
            ...
            'fecha_bautismo': self.fecha_bautismo.strftime('%Y-%m-%d') if self.fecha_bautismo else '',
...
class Comunion(models.Model):
    registro = models.OneToOneField('Registro', on_delete=models.CASCADE, related_name='comuniones') 
    id = models.AutoField(primary_key=True)
    no_libro = models.CharField(max_length=10, null=True, blank=True)
    no_folio = models.CharField(max_length=10, null=True, blank=True)
    no_acta = models.CharField(max_length=10, null=True, blank=True)
    ...
    def to_dict(self):
        return {
            ...
            'fecha_comunion': self.fecha_comunion.strftime('%Y-%m-%d') if self.fecha_comunion else '',
...
class Confirmacion(models.Model):
    registro = models.OneToOneField('Registro', on_delete=models.CASCADE, related_name='confirmaciones') 
    id = models.AutoField(primary_key=True)
    no_libro = models.CharField(max_length=10, null=True, blank=True)
    no_folio = models.CharField(max_length=10, null=True, blank=True)
    no_acta = models.CharField(max_length=10, null=True, blank=True)
    ...
    def to_dict(self):
        return {
            ...
            'fecha_confirmacion': self.fecha_confirmacion.strftime('%Y-%m-%d') if self.fecha_confirmacion else '',
            'obispo': self.obispo,
            'parroco': self.parroco,
...
class Matrimonio(models.Model):
    ...
    no_folio = models.CharField(max_length=10, null=True, blank=True)
    no_acta = models.CharField(max_length=10, null=True, blank=True)
    contrayente = models.OneToOneField('Registro', on_delete=models.CASCADE, related_name='contrayente')
    contrayenta = models.OneToOneField('Registro', on_delete=models.CASCADE, related_name='contrayenta')
    ...
    def to_dict(self):
        return {
            ...
            'fecha_matrimonio': self.fecha_matrimonio.strftime('%Y-%m-%d') if self.fecha_matrimonio else '',
            'parroco': self.parroco,
            'creacion': timezone.localtime(self.creacion).strftime('%Y-%m-%d %H:%M:%S') if self.creacion else '',
            'modificacion': timezone.localtime(self.modificacion).strftime('%Y-%m-%d %H:%M:%S') if self.modificacion else '',
            'estado': self.estado
