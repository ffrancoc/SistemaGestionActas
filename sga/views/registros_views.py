...
def tabla_registros(request):
    ...
    return render(
        request,
        "components/registros/listar_tabla.html",
...
def guardar_registro(request):
    ...
    if registro_id and str(registro_id).isdigit():
        try:
            ...
            registro.modificacion = timezone.now()
            registro.save()
...
def detalle_actas(request, registro_id):
    try:
        ...
        if registro:
            ...
            matrimonio = Matrimonio.objects.filter(
                Q(contrayente_id=registro.id) | Q(contrayenta_id=registro.id)
            ).first()
